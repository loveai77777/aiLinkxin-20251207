...
async function getPlaybooksByTag(tagSlug: string) {
  ...
  const tagSlugDecoded = decodeURIComponent(tagSlug);
  const tagLabelWithSpaces = tagSlugDecoded.replace(/-/g, " ");
  
  // Find tag by label or slug (case-insensitive)
  let { data: tagData } = await supabase
    .from("tags")
    .select("id, label, slug")
    .ilike("label", tagLabelWithSpaces)
    .maybeSingle();
  
  // If not found, try by slug
  if (!tagData) {
    const { data: altTagData } = await supabase
      .from("tags")
      .select("id, label, slug")
      .ilike("slug", tagSlugDecoded)
      .maybeSingle();
    tagData = altTagData;
  }
  ...
  if (!tagData) {
    const { data: partialTagData } = await supabase
      .from("tags")
      .select("id, label, slug")
      .ilike("label", `%${tagSlugDecoded}%`)
      .limit(1)
      .maybeSingle();
    tagData = partialTagData;
  }
  
  const tagDataFinal = tagData;

  if (!tagDataFinal) {
    return { playbooks: [], tagLabel: null };
  }

  // Fetch playbook IDs with this tag
  const { data: playbookTagsData } = await supabase
    .from("playbook_tags")
    .select("playbook_id")
    .eq("tag_id", tagDataFinal.id);

  if (!playbookTagsData || playbookTagsData.length === 0) {
    return { playbooks: [], tagLabel: tagDataFinal.label };
  }

  const playbookIds = playbookTagsData.map((pt) => pt.playbook_id);
  ...
  // Fetch all tags for these playbooks
  const { data: allPlaybookTagsData } = await supabase
    .from("playbook_tags")
    .select(
      `
      playbook_id,
  ...
  if (allPlaybookTagsData) {
    allPlaybookTagsData.forEach((pt) => {
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return createSeoMetadata({
    title: `#${tagLabel} - AI Playbooks`,
    description: `Browse AI Playbooks tagged with ${tagLabel}`,
  });
}

export default async function TagPage({ params }: PageProps) {
  const { tag } = await params;
  const { playbooks, tagLabel } = await getPlaybooksByTag(tag);
      ...
      <article>
        ...
        <header className="mb-8">
          ...
          <p className="text-xl text-white mb-6">
            {playbooks.length} {playbooks.length === 1 ? "playbook" : "playbooks"} with this tag
          </p>
        </header>
        ...
        {playbooks.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-white text-xl">No playbooks found with this tag.</p>
          </div>
        ) : (
